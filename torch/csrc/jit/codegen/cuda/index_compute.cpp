...
std::vector<Val*> Index::getGlobalProducerStridedIndices(
  ...
  for (const auto i : c10::irange(root_dom.size())) {
    ...
    if (root_ind->isZeroInt()) {
      ...
      if(!override_index.empty() && i != 0) { // modify later
        strided_ind = SimplifyingIrBuilder::modExpr(strided_ind, strides[i - 1]);
      }
...
std::vector<Val*> Index::getGlobalConsumerStridedIndices(
  ...
  for (const auto i : c10::irange(root_inds.size())) {
    auto override_it = override_index.find(root_dom[i]);
    if(override_it != override_index.end() || (!override_index.empty() && i == 0)) { // will modify later
      for(auto k : override_index) {
        root_inds[i] = k.second; // just hack, modify later
      }
    }
...
std::vector<Val*> Index::getNonGlobalConsumerStridedIndices(
  ...
  for (const auto i : c10::irange(root_dom.size())) {
    ...
    auto override_it = override_index.find(root_dom[i]);
    const bool is_overriden = override_it != override_index.end() || (!override_index.empty());
    auto item = override_it->second;
    for(auto k : override_index) {
      item = k.second; // just hack, modify later
    }
    auto root_ind_i =
        is_overriden ? item : index_map.at(root_dom[i]);
...
Val* Index::getConsumerStridedIndices(
  ...
  if (consumer->getMemoryType() == MemoryType::Global) {
    return sumVals(getGlobalConsumerStridedIndices(consumer, loops, override_index));
  } else {
    return sumVals(getNonGlobalConsumerStridedIndices(consumer, loops, override_index));
  }
}
...
kir::TensorIndex* Index::getConsumerIndex(
    ...
    const std::vector<kir::ForLoop*>& loops,
    const std::unordered_map<IterDomain*, Val*>& override_index) {
  auto strided_indices = getConsumerStridedIndices(consumer, loops, override_index);
  return SimplifyingIrBuilder::create<kir::TensorIndex>(
