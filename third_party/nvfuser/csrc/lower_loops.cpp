...
void LoopNestGenerator::generate(const std::vector<Expr*>& exprs) {
  ...
  for (auto tv : ir_utils::allTvs(FusionGuard::getCurFusion())) {
    ...
    for (auto tv_id : tv->domain()->domain()) {
      ...
      if (ca_map->idMap().find(concrete_id) != id_map.end()) {
        concrete_id = id_map[concrete_id];
      }
  ...
  for (auto tv : ir_utils::allTvs(FusionGuard::getCurFusion())) {
    ...
    auto id_map = ca_map->idMap();
    if (ca_map->idMap().find(last_id_concrete) != id_map.end()) {
      last_id_concrete = id_map[last_id_concrete];
    }
