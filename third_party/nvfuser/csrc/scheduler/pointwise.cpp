 ...
 public:
  ...
  TensorView* findReferenceTensorView(int minimum_num_axes = 0) const {
    ...
    for (auto output_tv :
      ...
      else if (isValidReference(output_tv) &&
          hasMinimumSize(output_tv, minimum_num_axes) &&
          !output_tv->isFusionInput()) {
        // for ScatterOp, for loop upper bound is generated by indexTv
...
void schedulePointwise(Fusion* fusion, const PointwiseParams& params) {
  ...
  for(auto x : fusion->outputs()) {
    std::cout << "out = " << x->toString() << std::endl;
  }
  ...
  std::cout << "before getReferenceTensorView " << std::endl;
  std::cout << fusion << std::endl;
  for(auto x : fusion->outputs()) {
    std::cout << "out = " << x->toString() << std::endl;
  }
